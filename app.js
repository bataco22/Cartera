...
$("assetForm").addEventListener("submit",e=>{e.preventDefault();if(!$("coinId").value||!state.selectedCoin){alert("Selecciona una cripto.");return}const lots=[...document.querySelectorAll(".lot-row")].map(r=>({amount:num(r.querySelector(".lot-amount").value),buyPrice:num(r.querySelector(".lot-price").value),buyCurrency:r.querySelector(".lot-currency").value,location:r.querySelector(".lot-location").value.trim(),purpose:r.querySelector(".lot-purpose").value||"long",targetPrice:num(r.querySelector(".lot-target").value),targetCurrency:r.querySelector(".lot-target-currency").value||"usd"})).filter(l=>l.amount>0);if(!lots.length){alert("Agrega al menos una cantidad mayor que cero.");return}const goals=[...document.querySelectorAll(".goal-edit-row")].map(r=>({label:r.querySelector(".goal-label").value.trim()||"Meta",price:num(r.querySelector(".goal-price").value),currency:r.querySelector(".goal-currency").value})).filter(g=>g.price>0);const a={coinId:$("coinId").value,name:state.selectedCoin.name,symbol:state.selectedCoin.symbol.toUpperCase(),lots,goals};const idx=$("editIndex").value;if(idx==="")state.assets.push(a);else state.assets[Number(idx)]=a;save();closeDialog();render();fetchPrices()});
